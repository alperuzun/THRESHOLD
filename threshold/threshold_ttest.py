...
def check_for_statistical_significance():
    ...
    if check_files(path+"stat1.txt",path+"stat2.txt") == True:
        ...
        def run_t_test(dataA, dataB, type_of_sat):
            ...
            t_stat, p_value = ttest_rel(df['Data A'], df['Data B'])

            n = len(df) 
            dof = n - 1  
            mean_diff = (df['Data A'] - df['Data B']).mean()
            squared_diff = (df['Data A'] - df['Data B'] - mean_diff)**2
            sum_squared_diff = squared_diff.sum()
            variance = sum_squared_diff/dof
            sd = variance**0.5\
            ...
            output = [type_of_sat,n, dof, mean_diff, sum_squared_diff, variance, sd, t_stat, p_value,interpretation]
            return output
